...
const ChatFilePage: React.FC<ChatFilePageProps> = ({
    ...
    return (
        <div
            className="flex flex-col h-full min-h-0 bg-[#FAFAFA] pb-6 px-6 relative"
        ...
        >
            {/* 스와이프 다운 인디케이터 (상단) */}
            <div
                className="w-full flex flex-col items-center py-4 cursor-pointer relative tap-feedback"
            ...
            {/* Main Display Card */}
            <div className="flex-1 min-h-0 w-full bg-white rounded-[32px] shadow-[var(--shadow-crisp)] relative overflow-y-auto overflow-x-hidden">
                <div className="w-full flex flex-col items-center pt-10 md:pt-16 pb-8">
                    <h2 className="text-[22px] font-bold text-center mb-8 md:mb-10 text-gray-900 tracking-tight leading-relaxed">
                        대화에 필요한 자료를<br />추가하시겠어요?
                    </h2>

                    <div className="grid grid-cols-3 gap-4 md:gap-6 z-10 w-full max-w-[640px] px-4 md:px-8">
                    {/* 파일 선택 */}
                    <label className="w-full aspect-square bg-[#F7F7F5] rounded-[24px] flex flex-col items-center justify-center gap-3 cursor-pointer tap-feedback active:scale-95 transition-transform hover:bg-[#F0F0ED]">
                    ...
                    {/* Upload Guidelines */}
                    <div className="mt-8 px-8 w-full max-w-[640px]">
